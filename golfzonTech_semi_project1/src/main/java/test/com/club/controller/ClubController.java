...
//         System.out.println("vo2:" + vo2);
         ...
         ClubVO vo3 = dao.leaderinfo(vo);//모임리더 정보를 출력합니다
//         System.out.println("vo3:" + vo3);
         request.setAttribute("vo3", vo3);

         ClubMemberDAO dao1 = new ClubMemberDAOimpl();
         List<ClubMemberVO> vos = dao1.clubmembers(vo);//모임 가입자 리스트를 출력합니다
//         System.out.println("vos:" + vos);
         request.setAttribute("vos", vos);

         List<ClubMemberVO> vos2 = dao1.waiting(vo);//모임가입 승인 대기 중인 리스트를 출력합니다
//         System.out.println("vos2:" + vos2);
         ...
         String query = dao2.createQuery(location, gender, age);//모임 가입 조건을 가져와 쿼리를 생성합니다
//         System.out.println(query);
         long club_id = Long.parseLong(request.getParameter("club_id"));

         boolean isQualified = dao2.isqualified(club_id, member_id, query);//모임 가입조건 충족여부
         
         String club_leader = vo3.getClub_leader();
         boolean isLeader = club_leader.equals(member_id);//접속자 ID가 모임리더ID인지 여부를 확인합니다
         ...
         dao4.selectStatus(vo5);//접속자의 모임 가입상태를 확인합니다. 미가입 또는 가입승인 대기 상태일 시 모임 내 게시판 숨기기 위함
...
//         System.out.println("vo4member_id:"+member_id);
         ...
         //접속자가 모임가입신청을 했는지 확인합니다. 회원 또는 가입승인 대기 상태일 때 가입신청 버튼 숨기기 위함
         boolean isMember = dao3.isMember(vo4);
...
/**
		...
		List<AlbumVO> album_list = new AlbumDAOimpl().selectAll(vo_album, "desc");
         ...
         request.setAttribute("isMember", isMember);//모임가입신청 여부
         request.setAttribute("isQualified", isQualified);//모임 가입조건 만족여부
...
//         System.out.println(request.getParameter("club_id"));
         ...
         ClubVO vo2 = dao.selectOne(vo);//모임정보를 불러옵니다
//         System.out.println("vo2:" + vo2);
...
//         System.out.println("member_id:" + member_id);
          ...
          * 2. 기능: 모임 하위 액티비티 삭제
          */
         int result_delete = new ActivityDAOimpl().deleteAll(vo);//모임 하위 액티비티 삭제
//		 System.out.println("result_delete: "+result_delete);
         
         int result = dao.delete(vo);//모임을 삭제합니다
//         System.out.println(result);
         if (result == 1)//삭제 성공하면 모임전체리스트로 이동합니다
            response.sendRedirect("club_selectAll.do");
         else//삭제 실패 시 모임 상세페이지로 이동합니다
...
//         System.out.println(location);
         ...
         //요청 검색키(지역, 나이대, 성별, 모임이름, 모임리더이름)에 맞는 쿼리문을 생성합니다
         String query = dao.createQuery_searchList(location, gender, age, searchKey);
         
         ClubDAO dao = new ClubDAOimpl();
         List<ClubVO> vos = dao.searchList(query,searchWord);//다중 검색을 실행합니다
//         System.out.println("vos.size():" + vos.size());
...
//         System.out.println("doPost:" + sPath);
         ...
         if (member_id != null) {
            ...
            String dir_path = request.getServletContext().getRealPath("/upload");//이미지파일 경로 지정
//            System.out.println(dir_path);

            ClubDAOimpl dao2 = new ClubDAOimpl();
            //club_id가 시퀀스값이여서 모임개설(모임테이블 입력)과 동시에 모임멤버테이블 입력 시 club_id가 안 맞는 문제가 발생하여
            //club_id를 가져오는 쿼리를 만들어 메소드 호출
               ...
               try {
                  ...
                  for (FileItem item : items) {
                     if (item.isFormField()) {
                        ...
                        if (item.getSize() != 0)
                           club_img = club_name + "_" + club_img;//파일명에 모임이름추가해서 파일이름구분
...
//                  System.out.println(request.getParameter("member_id"));
                  ...
                  dao.insertleader(vo1);//모임멤버 테이블에 모임리더 데이터 입력
...
//            System.out.println("result:" + result);
            ...
            if (result == 1)//수정값이 다 넘어오면 모임상세페이지로 이동
               response.sendRedirect("club_selectOne.do?club_id=" + club_id);
            else//누락된 값이 있으면 update창으로 redirect
               response.sendRedirect("club_update.do?club_id=" + club_id);
         } else//로그인값이 없으면 로그인 페이지로 이동
            request.getRequestDispatcher("HTML/login.jsp").forward(request, response);
